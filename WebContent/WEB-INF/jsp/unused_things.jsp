...
<body>
    ...
    <section class="breadcrumb-area bg-img bg-overlay" style="background-image: url(img/bg-img/27.jpg);">
        <div class="container h-100">
            <div class="row h-100 align-items-center">
                <!-- Breadcrumb Text -->
                <div class="col-12">
							  ...
							  <div class="col-lg-3 col-md-4 col-sm-6">
							    <div class="thumbnail">
							    	<c:if test="${article.featureImg!=null}">
							      		<div class="img-responsive bgDiv"  style="background-image: url(${article.featureImg});">
							    	</c:if>
							    	<c:if test="${article.featureImg==null}">
							      		<div class="img-responsive bgDiv"  style="background-image: url(img/unUpload2.png);">
							    	</c:if>
										<div class="userDiv" style="background-image: url(${article.author.user_headphoto});"> </div>							      	
							      </div>
							    <div class="caption">
							        <h3>${article.theme}</h3>
							        <p class="content">${article.content}</p>
							        <p class="concat"><a href="#" class="post-date">联系方式：</a> <span >${article.concat }</span> </p>
							        <p>  <button type="button" class="btn btn-primary" ><a href="viewArticle?articleId=${article.id}" class="detail">详情</a></button> <button type="button" class="btn btn-default" >${article.price}￥</button> </p>
							      </div>
							    </div>
							  </div>
							</c:forEach>  
	...
	$.get({
		...
		success:function(data){
			...
			for (var i = 0; i < data.length; i++) {
				var imgstr='<div class="img-responsive bgDiv"  style="background-image: url(img/unUpload2.png);">';
				if(data[i].featureImg!=null)
				{
					imgstr="<div class='img-responsive bgDiv'  style='background-image: url("+data[i].featureImg+");'>";
				}
				
				
				var str= ' <div class="col-lg-3 col-md-4 col-sm-6">'+
				   '<div class="thumbnail">'+imgstr+
						'<div class="userDiv" style="background-image: url('+data[i].author.user_headphoto+');"> </div>'+							      	
			      '</div>'+
			    '<div class="caption">'+
			        '<h3>'+data[i].theme+'</h3>'+
			        '<p class="content">'+data[i].content+'</p>'+
			        '<p class="concat"><a href="#" class="post-date">联系方式：</a> <span >'+data[i].concat+'</span> </p>'+
			     ' <p>  <button type="button" class="btn btn-primary" ><a href="viewArticle?articleId='+data[i].id+'" class="detail">详情</a></button> <button type="button" class="btn btn-default" >'+data[i].price+'￥</button> </p>'+
			     '</div>'+
			   '</div>'+
			  '</div>';
